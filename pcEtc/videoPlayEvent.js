...
export default async function videopPlayEvent(
  video,
  camCheck,
  faceBoard,
  firstResult
  ...
  setTimeout(async () => {
    ...
    interval = setInterval(async () => {
      const detections = await faceapi
        ...
        .withFaceLandmarks(true); //video 캡쳐본 얼굴인식 정보  사이즈를 맞추기 전이기때문에 좌표값이 다르다
      var resizedDetections;
      document.getElementById("loading").style.display = "none";
      if (detections) {
        resizedDetections = await faceapi.resizeResults(
          detections,
          displaySize
        ); //detections를 displaysize에 맞춘 결과값
        ...
        if (!faceBoard) {
          ...
          const leftCheek = {
            x: (nose.x + leftChin.x) / 2,
            y: (nose.y + leftChin.y) / 2,
          };
          const leftTopEye1 = positions[37];
          const leftBottomEye2 = positions[41];
          const leftPupil = {
            x: (leftTopEye1.x + leftBottomEye2.x) / 2,
            y: (leftTopEye1.y + leftBottomEye2.y) / 2,
          };
          const rightTopEye1 = positions[38];
          const rightBottomEye2 = positions[40];
          const rightPupil = {
            x: (rightTopEye1.x + rightBottomEye2.x) / 2,
            y: (rightTopEye1.y + rightBottomEye2.y) / 2,
          };
          const centerPupil = {
          ...
          var eyeAvgColor = context.getImageData(
            centerPupil.x,
            centerPupil.y,
            1,
            1
          ).data;
          var cheekAvgColor = context.getImageData(
            leftCheek.x,
            leftCheek.y,
            1,
            1
          ...
          ctx.putImageData(
            cheekPointArcCtx.getImageData(
              0,
              0,
              cheekPointArc.width,
              cheekPointArc.height
            ),
            leftCheek.x,
            leftCheek.y
          );
